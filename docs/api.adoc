...
The following scalars are supported starting from version 2.0.0 of GraphQL Lib:
...
Since version 2.0.0 of GraphQL Lib in order to create a schema you must to use a `schemaGenerator` object, which contains the following creation functions:
...
In order to create an instance of `schemaGenerator` enough to call the `newSchemaGenerator()` method of GraphQL Lib, for instance:
